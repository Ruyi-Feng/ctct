...
class Simu2:
    ...
    def _init_env(self) -> list:
        ...
        self.xlApp.Run(self.VBA_name_2)
        self.xlApp.Run(self.VBA_name_2)
        demand_GP = (self.sheet.Cells(3, 1).Value - 4.4) / (7.3 - 4.4)
        demand_ranp2 = (self.sheet.Cells(3, 2).Value - 0.38)/(2.2 - 0.38)
        den_up = (self.sheet.Cells(3, 3).Value - 4.4) / (7.3 - 4.4)
        den_down =(self.sheet.Cells(3, 4).Value) / 40.0
        den_rm2 = (self.sheet.Cells(3, 5).Value) / 350
        last_meter = self.sheet.Cells(3, 8).Value / 2.0
        return [demand_GP, demand_ranp2, den_up, den_down, den_rm2, last_meter]  # , last_meter
    ...
    def _sim(self, a: int) -> tuple:
        ...
        self.sheet.Cells(nk + 1, 8).Value = a * 0.5
        self.xlApp.Run(self.VBA_name_2)
        demand_GP = (self.sheet.Cells(nk + 1, 1).Value - 4.4) / (7.3 - 4.4)
        demand_ranp2 = (self.sheet.Cells(nk + 1, 2).Value - 0.38)/(2.2 - 0.38)
        den_up = (self.sheet.Cells(nk + 1, 3).Value - 4.4) / (7.3 - 4.4)
        den_down = self.sheet.Cells(nk + 1, 4).Value / 40.0
        den_rm2 = self.sheet.Cells(nk + 1, 5).Value / 350
        last_meter = self.sheet.Cells(nk + 1, 8).Value / 2.0
        s_= [demand_GP, demand_ranp2, den_up, den_down, den_rm2, last_meter] #, last_meter
    ...
    def run(self, aim_rtg=410):
        ...
        for i in tqdm(range(self.args.episode_num)):
            ...
            rtg = self._update_rtg(rtg, reward)
            a = self._update_a(a, action)
        DtcDenSum = self.SheetDen.Cells(1, 37).Value
        name = "aim%d_TTS_%d.xlsm"%(aim_rtg, int(DtcDenSum))
        excel_path = self.args.save_dir + name
        print("=====done=====")
        print("excel_path", excel_path)
...
class Simu3:
    ...
    def _init_env(self) -> list:
        ...
        self.xlApp.Run(self.VBA_name_2)
        self.xlApp.Run(self.VBA_name_2)
        demand_GP = (self.sheet.Cells(3, 1).Value - 4) / (7.5 - 4)
        demand_ranp2 = (self.sheet.Cells(3, 2).Value - 0.1)/(2.4 - 0.1)
        den_up = (self.sheet.Cells(3, 3).Value - 4) / (7.5 - 4)
        den_down =(self.sheet.Cells(3, 4).Value) / 40.0
        den_rm2 = (self.sheet.Cells(3, 5).Value) / 350
        last_meter = self.sheet.Cells(3, 8).Value / 2.0
        return [demand_GP, demand_ranp2, den_up, den_down, den_rm2, last_meter]  # , last_meter
    ...
    def _sim(self, a: int) -> tuple:
        ...
        self.sheet.Cells(nk + 1, 8).Value = a * 0.5
        self.xlApp.Run(self.VBA_name_2)
        demand_GP = (self.sheet.Cells(nk + 1, 1).Value - 4) / (7.5 - 4)
        demand_ranp2 = (self.sheet.Cells(nk + 1, 2).Value - 0.1)/(2.4 - 0.1)
        den_up = (self.sheet.Cells(nk + 1, 3).Value - 4) / (7.5 - 4)
        den_down = self.sheet.Cells(nk + 1, 4).Value / 40.0
        den_rm2 = self.sheet.Cells(nk + 1, 5).Value / 350
        last_meter = self.sheet.Cells(nk + 1, 8).Value / 2.0
        s_= [demand_GP, demand_ranp2, den_up, den_down, den_rm2, last_meter] #, last_meter
